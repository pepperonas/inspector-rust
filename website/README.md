# Product page — inspector-rust.celox.io

The app's website: <https://inspector-rust.celox.io>. Generated from
...
| `styles.css` | All styling. Colours are the app's own dark scheme (core/frontend/src/styles.css — @theme (dark)) as tokens on `:root`, from `site.json` → `theme`. |
...
`--delete`): `latest.json`, `changelog.md`, `ssi/`, `/etc/nginx/inspector-rust-download.conf`.

## How "always the newest release" works

`inspector-rust-latest.timer` runs `inspector-rust-latest.py` every 15 minutes (and 2 minutes after boot). It asks the
...
```bash
ssh root@69.62.121.168 'systemctl start inspector-rust-latest.service; journalctl -u inspector-rust-latest -n 3 -o cat'
# inspector-rust-latest: v1.11.0 json=new ssi=new changelog=new nginx=reloaded
```
...
| What | Where | Notes |
|------|-------|-------|
| Newest release | `https://inspector-rust.celox.io/download`, `/download/<target>` | 302 to the current GitHub release asset — `/download` for the visitor's platform. The download button's `href` in the HTML is `/download`; JavaScript picks the platform in the browser and swaps in the direct asset URL. |
...
```bash
curl -sI https://inspector-rust.celox.io/ | grep -i -E '^link|^vary'
curl -s  -H 'Accept: text/markdown' https://inspector-rust.celox.io/ | head -20
curl -s  https://inspector-rust.celox.io/latest.json
```
...
- The language comes from `navigator.languages` (first of en/de/es/it/fr), is remembered in
  `localStorage` (`inspector-rust-lang`) and switched with the flag menu (keyboard: arrows, Home/End, Enter,
...
./website/deploy.sh server   # also: inspector-rust-latest.py, its units, the vhost, nginx reload
```
...
2. **Certificate:** put an HTTP-only vhost in place that serves `/.well-known/acme-challenge/` from
   ...
   `certbot certonly --webroot -w /var/www/html -d inspector-rust.celox.io -d <alias> … --deploy-hook "systemctl reload nginx"`.
3. `mkdir -p /var/www/inspector-rust.celox.io`, then `./website/deploy.sh server`.
...
```bash
B=https://inspector-rust.celox.io
...
echo | openssl s_client -connect inspector-rust.celox.io:443 2>/dev/null | openssl x509 -noout -ext subjectAltName -enddate
ssh root@69.62.121.168 'systemctl list-timers inspector-rust-latest.timer --no-pager'
```
