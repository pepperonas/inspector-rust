...
- **State:** `RecordState { Mutex<Option<Session>> }` (managed Tauri state); `Session { region, audio, ffmpeg, final_out, segments: Vec<PathBuf>, current: Option<(Child, PathBuf)>, seq }` — `current` is `None` while paused. The pure arg-builders (`build_args_macos`/`build_args_windows`) + device parsers + concat-list builder are unit-tested (10 tests, no ffmpeg needed). **Caveats:** ffmpeg must be installed; system audio needs a loopback device (macOS: BlackHole); macOS may prompt ffmpeg for its own Screen-Recording TCC the first time. IPC: `screen_record_open_overlay`, `cancel_record_overlay`, `start_screen_record`, `pause_screen_record`, `resume_screen_record`, `stop_screen_record`, `is_recording`. Frontend wrappers in `ipc.ts`; both windows routed in `main.tsx` and listed in each `capabilities/default.json`.
