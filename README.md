...
<div align="center">
  ...
  *Roughly sorted by everyday usefulness × how much engineering sits behind it — flagship features first, easter eggs last.*
  ...
  Tauri 2 (WebView2 / WKWebView) · Rust workspace (`core/rust-lib` shared, 2-line per-OS bundle shells) · React 19 + TypeScript 5 + Tailwind v4 + Vite 7 · brightness via CoreGraphics/GDI gamma + DDC/CI (`ddc-hi`). **1776 unit tests (808 Rust + 968 frontend).** MIT-licensed.
  ...
  <p>
    ...
    &nbsp;
    <a href="https://github.com/pepperonas/inspector-rust/actions/workflows/ci.yml" title="Unit tests — 808 Rust + 968 frontend, all passing">
  ...
  [![cargo test](https://img.shields.io/badge/cargo%20test-647%20passing-success?style=flat-square&logo=rust&logoColor=white)](#)
  [![vitest](https://img.shields.io/badge/vitest-797%20passing-success?style=flat-square&logo=vitest&logoColor=white)](#)
...
| **Power command — `sec` / `nmap` / `sqlmap` / `ferox` / `john`** *(v0.84.271+)* | Search bar | Guided pentest-command builders — presets → shell-quoted command line + flag cheat-sheet. Enter copies; ⌘/Ctrl+Enter opens your terminal (macOS, opt-in). Never scans itself. Authorized targets only. |
...
| **Disk cleaner** *(v0.60.0+)* | Type `clean` (alias `cleanup`) → scans an allow-listed set of cache/log/temp roots → pick categories in the preview panel (checkboxes, sizes, largest files) → delete; Safe / Standard / Aggressive levels in Settings. Standard sweeps the whole user cache dir; Aggressive (opt-in) adds dev caches, Xcode DerivedData and old Trash | core |
