...
5. **Docs** — document it in this file (`CLAUDE.md`), add it to the README command lists (`README.md` + `README.de.md`: the *Power commands* bullet + the `search-bar commands` count badge), **and add a one-line entry to `features.txt`**.

Steps 4 + 5 are not optional: **every new custom command must appear in the project docs, `features.txt`, and the app's Features tab.**
...
| Table | Purpose | Notes |
|---|---|---|
| `entries` | Clipboard history | SHA-256 deduped; capped at 1 000 rows via `prune_locked`; sorted by `pinned DESC, last_used_at DESC`. **Pinned clips (v0.76.0)**: a `pinned` column (lazy `ALTER TABLE` migration) exempts a row from the prune (`WHERE pinned = 0`) and floats it to the top. `db::set_pinned` + IPC `set_clip_pinned`; the pin ★ toggle lives on each `HistoryItem` row. **Clip notes (v0.84.106)**: a `note` column (lazy `ALTER TABLE`, AES-encrypted at rest like content) attaches a user note to a clip; noted rows are **highlighted yellow** in the list (+ note icon, text on hover) and are **exempt from the prune** too (`WHERE pinned = 0 AND note IS NULL`, both subquery + outer). `db::set_note` (empty string clears) + IPC `set_clip_note`; the note button sits top-right in `PreviewPanel`'s `meta` row (`NoteButton`: add / edit / delete popover, yellow when set). Notes round-trip through backup (restored after `upsert_clip` via `set_note`/`set_pinned`). **Slim list (v0.84.16)**: the popup history *view* uses `db::list_slim`, which blanks `content_data` for `image` rows (the list renders an icon, not the bitmap, and image actions + paste re-read by id) — image clips hold multi-MB base64 PNGs, and shipping the full set on every popup-open / clipboard-change was decrypting + JSON-serialising + IPC-marshalling **~140 MB** (the "overlay loads slowly" bug). `get_history`/`search_history` use `list_slim`; the preview fetches a selected image's pixels on demand via the `get_clip` IPC (`db::get`). The full `db::list` (with image data) is retained for the backup export, which needs every byte. |
| `snippets` | Text expander templates | `abbreviation` + `title` + `body`; index on `abbreviation`. **Curated AI-prompt seed (`seed.rs`)**: 27 hand-written `ai*`-prefixed prompt templates in `seed/ai_prompts.json` are imported on first run (`maybe_seed_defaults`, guarded by `seed.default_snippets_v1`) and re-importable via the Snippets-tab "Restore defaults". Seed tests assert the count, `ai`-prefix, uniqueness, non-empty bodies. **Material-Design colour pack (v0.84.127)**: 255 `shortcut → hex` snippets in `seed/material_colors.json` (e.g. `reda700` → `D50000`, hex without `#`) are seeded the same way behind a **separate** flag (`seed.material_colors_v1`) so existing installs receive them on the next launch; `import_pack` does both, `restore_defaults` restores both. Tests assert count=255, unique lowercase shortcuts, 6-hex-digit bodies. |
