...
### boom — system-wide audio EQ (`boom/`, `boom-driver/`, v0.84.143–.157; macOS)

`boom` opens an inline **audio-enhancement controller** in the right preview column (same inline family as `stats`/`hue`; `boomMode`/`boomFocus` in `App.tsx`, macOS-gated `CommandSpec.platform=["mac"]`). Working system-wide EQ + presets + volume boost. **Needs a one-time virtual-driver install** (NOT driverless — see the history note; that's a real macOS limitation, not a choice).
