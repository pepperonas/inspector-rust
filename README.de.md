...
<div align="center">
  ...
  *Grob sortiert nach Alltagsnutzen × Engineering-Aufwand dahinter — Flagship-Features zuerst, Easter-Eggs zuletzt.*
  ...
  Tauri 2 (WebView2 / WKWebView) · Rust-Workspace (`core/rust-lib` geteilt, 2-Zeilen-Per-OS-Bundle-Shells) · React 19 + TypeScript 5 + Tailwind v4 + Vite 7 · Helligkeit via CoreGraphics/GDI-Gamma + DDC/CI (`ddc-hi`). **1776 Unit-Tests (808 Rust + 968 Frontend).** MIT-lizenziert.
  ...
  <p>
    ...
    &nbsp;
    <a href="https://github.com/pepperonas/inspector-rust/actions/workflows/ci.yml" title="Unit-Tests — 808 Rust + 968 Frontend, alle grün">
      <img src="https://img.shields.io/badge/unit%20tests-1776%20passing-2ea043?style=for-the-badge&logo=vitest&logoColor=white" height="64" alt="Unit tests" />
    </a>
  </p>
  ...
  [![Unit tests](https://img.shields.io/badge/unit%20tests-1776%20(808%20Rust%20%2B%20968%20TS)-success?style=flat-square)](https://github.com/pepperonas/inspector-rust/actions/workflows/ci.yml)
  ...
  <!-- ── Features (numerical) ────────────────────────────────── -->
  [![Tests](https://img.shields.io/badge/tests-1776%20passing-success?style=flat-square)](#)
...
| **Power-Command — `sec` / `nmap` / `sqlmap` / `ferox` / `john`** *(v0.84.271+)* | Suchfeld | Geführte Pentest-Command-Builder — Presets → shell-gequotete Kommandozeile + Flag-Erklärungen. Enter kopiert; ⌘/Ctrl+Enter öffnet dein Terminal (macOS, opt-in). Scannt nie selbst. Nur autorisierte Ziele. |
...
| **2FA / TOTP-Manager** *(v0.47.0+)* | `2fa` oder `otp` tippen → Enter öffnet den TOTP-Tresor: Live-Codes + Countdown, **Hinzufügen / Bearbeiten (inkl. Secret) / Löschen, Drag-Umsortieren (⠿-Griff), Duplikate entfernen / Alle löschen**. Import per Einfügen **oder Datei-Drag&Drop** (Google-Auth-Migration · Aegis · 2FAS · OTPManager · otpauth), dedupliziert beim Import. **Tippen filtert die Liste** — Fuzzy-Match über Issuer/Account, Top-Treffer umrandet, Enter kopiert dessen Code + schließt das Popup, Esc leert erst den Filter. `otp <issuer>` / `2fa <issuer>` vervollständigen einen Code inline. Secrets AES-verschlüsselt, überqueren nie IPC | core |
...
| **Aufräum-Tool** *(v0.60.0+)* | `clean` (Alias `cleanup`) → scannt eine Allowlist von Cache-/Log-/Temp-Verzeichnissen → Kategorien im Vorschau-Panel wählen (Checkboxen, Größen, größte Dateien) → löschen; Safe / Standard / Aggressive in Settings. Standard fegt das komplette Nutzer-Cache-Verzeichnis; Aggressive (opt-in) ergänzt Dev-Caches, Xcode-DerivedData und alte Papierkorb-Einträge | core |
...
```bash
pnpm test               # Frontend-Unit-Tests (vitest + happy-dom) — 968 Tests
cargo test --workspace  # Rust-Unit-Tests — 808 Tests
```
