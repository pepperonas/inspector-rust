...
Assembly order in `App.tsx` (`combined`): **runnable command first, then command suggestions (highest priority, v0.72.0/v0.74.0)** → opener → app-launcher → bruno/pwgen/bpm/totp special rows → calc result → color result → snippet matches → fuzzy clips. Both the complete `commandEntry` **and** the partial command `suggestionEntries` are spliced **above** the app-launcher hit so typing `terminal` (or even the partial `term`) surfaces the custom command (open a terminal in the current Finder folder) above Terminal.app — an app-name fuzzy-match must never outrank a custom command (complete or suggested).
...
- **DSP engine (`boom/mod.rs`, pure + unit-tested):** the realtime signal chain — **pre-amp → 10-band graphic EQ (RBJ peaking biquads, transposed DF-II) → volume boost → soft-knee limiter**. `Biquad` (0 dB = exact passthrough), `DspChain::process_interleaved` (allocation-free hot path), `soft_limit` (transparent below 0.8·ceiling, smoothly compresses above → never clips/blows speakers), 20 presets (15 genre + 5 device-correction), `BoomConfig` (persisted as JSON under settings `boom.config`; clamped by `normalize`). 11 unit tests (biquad boost/cut, limiter ceiling, flat-chain passthrough, boost-then-limit, preset/normalize, version gate). IPC: `boom_available` · `boom_presets` · `get_/set_boom_config`.
...
- **TODO:** Developer-ID signing + notarization for distribution, output-device-change + sample-rate-change rebuild, residual clock-drift correction (rare click over long sessions). Phase 2: 31-band EQ, enhancement effects (bass synth/clarity/ambience/night — the sliders exist, DSP TODO), device auto-detect.
...
**Windows: confirm-after-settle (v0.84.131).** The fixed-grace + instant-PID-snapshot scheme below kept losing the race — the WebView2 bounce fires `Focused(false)` 700–900 ms post-show (past any fixed grace), and a single `GetForegroundWindow` snapshot can hit the instant the foreground is momentarily not ours → the popup "opens then closes by itself". The durable fix **stops deciding synchronously on Windows**: a `Focused(false)` (once the popup has been focused at least once — `popup_was_focused`) schedules `hotkey::schedule_settle_hide`, which re-evaluates ~250 ms later (`SETTLE_MS`) on the main thread via the pure `settle_should_hide(captured_gen, current_gen, suppressed, foreground_is_ours, visible)`. A monotonic `SHOW_GEN` (bumped by `mark_shown` + `mark_popup_focused`) is captured at the bounce and compared at settle-time — so any intervening re-show or focus-regain (a *resolving* bounce) cancels the hide, regardless of when the bounce arrives. A real click-away keeps a foreign window foreground past the settle → hide proceeds. A `Focused(false)` **before** the first `Focused(true)` is the `SetForegroundWindow`-failed show-race → ignored. **Foreground hardening:** `show_and_position` on Windows calls `force_foreground` (the `AttachThreadInput` trick: attach to the current foreground thread → `SetForegroundWindow`+`BringWindowToTop` → detach) so the popup reliably activates from the background tray process instead of showing behind. `settle_should_hide` is unit-tested; macOS/Linux keep the immediate-hide path below (no focus bounce there).
