...
Pure, unit-tested mappings: `percent_to_raw`/`raw_to_percent` (DDC, scaled to each device's `maximum()`), `percent_to_gamma_fraction` (macOS/Windows; clamps to a `MIN_PERCENT=10` floor so the screen can never dim to unrecoverable black), and `gamma_ramp_entry` (Windows 16-bit ramp entry; `index*257*fraction`). The applied level is tracked **in-process** per display (gamma has no "read current backlight"); it resets to 100% at logout. Re-enumeration preserves the current dim level so re-opening the overlay doesn't show a reset 100%. Caches are platform-specific (`Vec<Entry>` for the gamma paths, `Mutex<Option<DisplayCache>>` for DDC); the frontend debounces writes ~80 ms and the slider min is 10. `supports_ddc` now means "controllable" (always `true` on the gamma paths; on Linux a non-answering monitor is `false` and hidden). IPC: `list_brightness_monitors`, `get_monitor_brightness`, `set_monitor_brightness`, `brightness_open` (hides popup + builds/shows the interactive, focusable, frameless, transparent, always-on-top `brightness-overlay` window — listed in each `capabilities/default.json`), `brightness_close`. Frontend: `BrightnessOverlay.tsx` (routed in `main.tsx` by the `brightness-overlay` label). **Caveat:** software dimming reduces emitted light, not backlight — it can only go *darker* than native, never brighter.
