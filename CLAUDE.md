...
### boom — audio enhancement (`boom/`, v0.84.143; macOS, driverless)

`boom` opens an inline **audio-enhancement controller** in the right preview column (same inline family as `stats`/`hue`; `boomMode`/`boomFocus` in `App.tsx`, macOS-gated `CommandSpec.platform=["mac"]`). System-wide EQ + presets + volume boost, **driverless** via Apple's **Core-Audio process taps** (macOS 14.2+) — no BlackHole/kext.

- **Phase 1a (`boom/mod.rs`, the pure unit-tested DSP engine + config):** the signal chain the realtime IOProc will run — **pre-amp → 10-band graphic EQ (RBJ peaking biquads, transposed DF-II) → volume boost → soft-knee limiter**. `Biquad` (0 dB = exact passthrough), `DspChain::process_interleaved` (allocation-free hot path), `soft_limit` (transparent below 0.8·ceiling, smoothly compresses above → never clips/blows speakers), 20 presets (15 genre + 5 device-correction), `BoomConfig` (persisted as JSON under settings `boom.config`; clamped by `normalize`). `is_supported()` gates on macOS 14.2+ (pure `version_ge_14_2`). All pure cores unit-tested (biquad boost/cut, limiter ceiling, flat-chain passthrough, boost-then-limit, preset/normalize, version gate). IPC: `boom_available` · `boom_presets` · `get_/set_boom_config`. Frontend: `BoomPanel.tsx` (on/off, preset dropdown, pre-amp, 10 vertical EQ sliders, boost slider with 100 % tick + over-100 % warning, enhancement-effect sliders) with a banner that the **live routing lands in phase 1b**.
- **Phase 1b (`boom/macos.rs`, v0.84.144 — live engine, SAFE-FIRST):** `CATapDescription` (objc2: `initStereoGlobalTapButExcludeProcesses:[]` → global tap, `setPrivate:true`) → `AudioHardwareCreateProcessTap` → private aggregate device (raw-CF dict: `private`/`taps`/`subdevices`/`master`, default output as main sub-device) → `AudioDeviceCreateIOProcIDWithBlock` realtime IOProc (block2; opaque `c_void` args cast inside — block2 needs `Encode` args) that copies the tapped input → output and runs `DspChain` in place (params via `Mutex<DspChain>` `try_lock` → a contended tweak passes that block through). `set_active(cfg)` starts/stops + pushes params (rebuilds the chain at the device's nominal sample rate); `shutdown()` tears down (idempotent). **`MUTE_BEHAVIOR` starts UNMUTED** so a bug can never silence the Mac (worst case: doubled audio) — flip to muted (1) once verified on real hardware. Cleanup runs on disable; process taps are per-process so a crash/quit lets CoreAudio reclaim the tap. `boom::apply(db)` (after `set_boom_config` + at startup) syncs the engine. **Still TODO before flipping to MUTED:** explicit app-exit teardown hook, the TCC audio-capture permission UX, RMS watchdog (all-zero-stream rebuild), sample-rate / output-device change rebuild.
