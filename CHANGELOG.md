...
- **The overlay sometimes didn't appear on the hotkey.** With "close on click outside" off, the overlay can stay open but unfocused — behind other windows or on another screen. The hotkey saw it as open and *closed* it, so the press seemed to do nothing. It now only closes a focused overlay; otherwise it brings it to the front on the screen under the cursor.
- **The version number in the footer now shows reliably, at the bottom right.** It was read through a Tauri plugin permission that could be refused silently, and then nothing was shown.
...
- **`loc`-Report exportieren — HTML, PDF und PNG.** Drei Knöpfe über der Statistik; die Datei landet in `~/Downloads` und wird im Finder gezeigt. **Ein einziger Renderer in Rust** erzeugt alle drei Formate (die HTML geht unverändert raus, durch WebKit als PDF und als Ganzseiten-Schnappschuss als PNG) — so können sie nicht auseinanderlaufen. Das Dokument ist eigenständig: eingebettetes CSS, kein Skript, keine Netz-Anfrage.
...
- **Field note (after release): if a gesture *still* seems to double-fire, check for an external gesture tool.** The final piece of this hunt turned out to be **BetterTouchTool running with the same bindings** (3-finger tap → Mute, 3-finger swipes → volume) on the test machine — every tap fired BTT's action *and* Inspector Rust's, a double no in-app fix could remove. Inspector Rust logs one `gesture dispatch:` INFO line per action; if the log shows one dispatch but the effect doubles, the second actor is another tool (BetterTouchTool, Karabiner-Elements, …) — disable its overlapping binding.
...
- **Binary size.** Measured **+3.7 MB** on the release binary (48.56 → 52.29 MB): the compressed 550-font bundle is 2.3 MB, the rest is figlet-rs + flate2 + a transitive `zip` crate + the generated blob table. New minor because it's a substantial user-facing feature; no breaking changes.
...
- **History list — styled-text indicator.** Clips that carry **styled** content
  (HTML / RTF) now show a subtle muted **HTML / RTF** tag next to the entry, so
  they're distinguishable at a glance from **plain text** (which shows no tag).
