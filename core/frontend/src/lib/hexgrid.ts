...
export function hexCenters(cols: number, rows: number, w: number, h: number): HexCell[] {
  ...
  // Pointy-top tessellating honeycomb: rows are offset by half a hex and overlap
  // vertically by 1/4 (row pitch = 0.75·hexHeight) so the hexes interlock into a
  // real honeycomb. A half-cell of width is reserved for the offset rows; a
  // quarter-cell top + bottom for the points.
  const wHex = w / (cols + 0.5);
  const hHex = h / (0.75 * rows + 0.25);
  const vStep = 0.75 * hHex;
  for (let row = 0; row < rows; row++) {
    const offset = (row % 2) * (wHex / 2);
    for (let col = 0; col < cols; col++) {
      out.push({ col, row, cx: offset + (col + 0.5) * wHex, cy: hHex / 2 + row * vStep });
    }
  }
  return out;
}
...
/**
 * SVG `points` string for a **pointy-top** hexagon (a point at top + bottom,
 * flat left + right sides — the classic honeycomb cell) centered at `(cx, cy)`
 * with radii `rx`/`ry`. First vertex points up (−90°), then every 60°.
 */
