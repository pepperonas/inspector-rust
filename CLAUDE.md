...
- **Frontend:** `BoomPanel.tsx` (on/off, preset dropdown, pre-amp, 10 vertical EQ sliders, boost slider with 100 % tick + over-100 % warning, enhancement-effect sliders) + a banner explaining the driver requirement.
...
- **TODO (B4):** in-app first-run driver install/uninstall UX (currently CLI scripts), Developer-ID signing + notarization for distribution, output-device-change + sample-rate-change rebuild, residual clock-drift correction (rare click over long sessions). Phase 2: 31-band, enhancement effects (bass synth/clarity/ambience/night), device auto-detect, live level meters.
