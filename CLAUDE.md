...
- **Phase 1b (`boom/macos.rs`, planned):** `CATapDescription` (empty process list, global, `muteBehavior=MUTED`) → `AudioHardwareCreateProcessTap` → private aggregate device (`kAudioAggregateDeviceIsPrivateKey`) with the tap + the default output as main sub-device → `AudioDeviceCreateIOProcIDWithBlock` realtime callback running `DspChain` → real output. Plus: TCC audio-capture permission flow, RMS watchdog for the all-zero-stream bug (full teardown+rebuild), sample-rate-change + output-device-change rebuild, and **bulletproof cleanup so the Mac is never left muted**.
