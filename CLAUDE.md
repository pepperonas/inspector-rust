...
**Windows: confirm-after-settle (v0.84.131).** The fixed-grace + instant-PID-snapshot scheme below kept losing the race — the WebView2 bounce fires `Focused(false)` 700–900 ms post-show (past any fixed grace), and a single `GetForegroundWindow` snapshot can hit the instant the foreground is momentarily not ours → the popup "opens then closes by itself". The durable fix **stops deciding synchronously on Windows**: a `Focused(false)` (once the popup has been focused at least once — `popup_was_focused`) schedules `hotkey::schedule_settle_hide`, which re-evaluates ~250 ms later (`SETTLE_MS`) on the main thread via the pure `settle_should_hide(captured_gen, current_gen, suppressed, foreground_is_ours, visible)`. A monotonic `SHOW_GEN` (bumped by `mark_shown` + `mark_popup_focused`) is captured at the bounce and compared at settle-time — so any intervening re-show or focus-regain (a *resolving* bounce) cancels the hide, regardless of when the bounce arrives. A real click-away keeps a foreign window foreground past the settle → hide proceeds. A `Focused(false)` **before** the first `Focused(true)` is the `SetForegroundWindow`-failed show-race → ignored. **Foreground hardening:** `show_and_position` on Windows calls `force_foreground` — (1) temporarily set the **foreground-lock timeout to 0** (`SystemParametersInfoW(SPI_SETFOREGROUNDLOCKTIMEOUT, 0)`, read+restore the user's value, flags 0 = in-memory only), then (2) the `AttachThreadInput` trick (attach to the current foreground thread → `SetForegroundWindow`+`BringWindowToTop` → detach). **The `SPI_SETFOREGROUNDLOCKTIMEOUT` step (v0.84.201) is the fix for "after a while, opening the app closes it immediately" (Windows-only):** once Windows' foreground-lock timeout has elapsed (the user has interacted with *other* apps for a while), a background/tray process's `SetForegroundWindow` is silently refused — `AttachThreadInput` alone doesn't beat the *timeout* — so the popup `show()`s but never activates, the settle-confirm sees a foreign foreground, and hides it. A fresh boot/relaunch temporarily resets the lock, which is why a restart appeared to fix it. Zeroing the timeout around the activation makes the take-foreground reliable. `settle_should_hide` is unit-tested; macOS/Linux keep the immediate-hide path below (no focus bounce there). Isolated `cargo check --target x86_64-pc-windows-gnu` validated the FFI.
