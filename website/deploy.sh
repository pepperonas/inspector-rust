#!/usr/bin/env bash
# Deploy the product page to inspector-rust.celox.io (generated from templates/apps/product-page).
...
if [[ "${1:-}" == server ]]; then
  ...
  scp server/nginx/inspector-rust.celox.io "$HOST:/etc/nginx/sites-available/inspector-rust.celox.io"
  ssh "$HOST" "chmod 755 /usr/local/sbin/$SLUG-latest.py &&
    ln -sf /etc/nginx/sites-available/inspector-rust.celox.io /etc/nginx/sites-enabled/ &&
