...
export const COMMAND_DOCS: CommandDoc[] = [
  ...
  },
  {
    ...
    tagline: "Bluetooth protocol analyzer — live BLE scan + import/decode captures, byte-diff.",
    tagline_de:
      "Bluetooth-Protokoll-Analyzer — Live-BLE-Scan + Captures importieren/dekodieren, Byte-Diff.",
    synopsis: "btsniff   ·   btcap · btcapture · blecap",
    description:
      "A small Bluetooth protocol analyzer in the preview. Two modes: (1) **Live BLE scan** (macOS) — press *Capture starten* and CoreBluetooth logs every advertising device (identifier, name, RSSI, service UUIDs, manufacturer-data bytes) as a live packet, with a LIVE clock, pause/resume/stop/clear and auto-scroll; (2) **Import** a `.pklg` (macOS PacketLogger), `btsnoop_hci.log` (Android / BlueZ) or `.pcapng` (Wireshark) capture and decode HCI / ACL / L2CAP / ATT / GATT / SMP. Both feed one virtualised timeline (direction → TX / ← RX, protocol, handle, UUID); picking a packet opens an inspector with decoded fields, a hex dump, and a **byte-diff against the previous packet on the same handle** — so a value that keeps changing stands out. Filter by layer or direction, full-text search, JSON export. Distinct from the `bt` device manager — this analyses protocol data, it does not pair devices.",
    arguments: [],
    flags: [],
    examples: [
      { input: "btsniff", result: "Opens the analyzer — start a live BLE scan or import a capture file." },
      { input: "btcap", result: "Same command, short form." },
      { input: "btsniff?", result: "This help." },
    ],
    tips: [
      "The byte-diff highlights exactly the bytes that changed since the same device's last packet — the fastest way to find the payload field a beacon or an app is toggling.",
      "Filter to GATT to see only the value-bearing reads/writes/notifications and skip the connection-setup chatter.",
      "Pause freezes the VIEW only — the backend keeps capturing, and Resume flushes the buffered packets, so nothing is lost while you inspect.",
      "Drag a `btsnoop_hci.log` straight from Finder onto the panel to import it.",
    ],
    caveats: [
      "Live capture is a **BLE advertisement scanner**, not a wire sniffer of another app's traffic — macOS exposes no public promiscuous-HCI capture API. To reverse a phone app ↔ device session, record a `btsnoop_hci.log` on Android and import it here.",
      "Live scan is macOS-only (CoreBluetooth) and needs Bluetooth on + the Bluetooth permission; other platforms get a clean message and full file-import.",
      ...
      "Export is JSON (the decoded analysis); the export filename is timestamp-only and any custom name is stripped of MAC-address runs — a device address never lands in a filename.",
    ],
  ...
  },
  {
    ...
    flags: [],
    examples: [
      { input: "dezibel", result: "Enter opens the meter; the number tracks the room live." },
      { input: "db", result: "Short form of the same command." },
  ...
  },
  {
    ...
    tagline: "Git repo stats + clone — paste a GitHub URL, get ranges, heatmap, hotspots, bus factor.",
    tagline_de: "Git-Repo-Statistik + Klonen — GitHub-URL einfügen: Zeiträume, Heatmap, Hotspots, Bus-Faktor.",
    synopsis: "repo [url|pfad]   ·   export [url|pfad]",
    description:
      "Analyses a git repository's history and shows it in the preview, oriented on the repo2viz project: KPI tiles (commits, contributors, active days, longest streak, lines added/removed), a month-activity sparkline, weekday and hour-of-day charts, commit-category bars (conventional commits), and the most-active files, file types and contributors. Give a GitHub URL (read-only bare clone), a local path, or nothing — then it uses the folder selected in Finder if it's a git repo. A bare GitHub repo URL typed or pasted into the search bar gets its own **Repo analysieren** row — no `repo` keyword needed (issue/PR links are ignored); a clip containing one shows the same button in its preview. Chips switch between **30 T / 90 T / 180 T / 1 J / Gesamt** instantly — ranges count back from the newest commit, not from today. At the very top, **Aktivität 24 h / 7 Tage** shows commits, lines, files, contributors and tags for the last 24 hours and 7 days (plus pushes, PRs and issues from the GitHub API for GitHub repos), each compared with the period before. Right under them, **Code-Änderungen** shows lines added / removed / net with a per-step bar chart; the time steps follow the range (days for 30 T, weeks for 90/180 T, months for 1 J / Gesamt). Extra cards: weekday × hour **heatmap**, a GitHub-style **contribution calendar** (1 J / Gesamt), **hotspots** (often changed, ≤ 2 authors), **bus factor** overall and per top-level folder, and **co-change** pairs. **⌘K** clones the repo into the clone folder (Settings → Repositories); an occupied name becomes `name (2)`, `name (3)` … — nothing is overwritten. ⌘E / ⌘P export the selected range as HTML / PDF to ~/Downloads without re-cloning.",
    arguments: [
    ...
    examples: [
      ...
      { input: "https://github.com/pepperonas/repo2viz", result: "Repo row without any keyword — Enter analyses it." },
      { input: "repo https://github.com/o/r  → ⌘K", result: "Clones into ~/claude/r (or r (2) when taken) and reveals it." },
    ],
    tips: [
      "In the panel, ⌘E / ⌘P export the selected range as HTML / PDF; ⌘K clones the repo.",
      "Private repos work when `gh auth login` is done, or with a token in Settings → Repositories (kept in the keychain, never on a command line).",
      "Pushes count all branches, the git numbers only the main branch; the Events API returns at most 300 events (then the count reads ≥ n) and reports pushes with a delay of up to a few hours.",
    ...
    caveats: [
      "A GitHub URL is cloned once into a cache (max. 5 repos / 2 GB) — the next analysis only needs a `git fetch`; the first one of a very large repo takes a while.",
      "Needs git on PATH.",
    ],
    related: ["loc", "disk"],
  ...
  },
  {
    ...
    caveats: [
      ...
      "An empty folder shows a plain \"empty\" state, not a chart. `disk /home` is special-cased: macOS's `/home` is an empty `autofs` mount, so it auto-opens your real home folder (`~`) instead.",
      "A folder you can't read (macOS: missing Full Disk Access) shows a \"Kein Zugriff\" card with a one-click **Full Disk Access öffnen** button — it isn't empty, the app just can't see into it.",
    ],
    related: ["loc", "stats"],
