...
export const COMMAND_DOCS: CommandDoc[] = [
  ...
  // ── Productivity ──────────────────────────────────────────────────────
  {
    ...
    tagline: "German gross→net salary calculator (tax year 2025).",
    tagline_de: "Brutto-Netto-Rechner Deutschland (Steuerjahr 2025).",
    synopsis: "bruno <€>[m|j]",
    description:
      "Computes German net pay from a gross amount (simplified §32a tariff, tax year 2025). Suffix `m` = monthly, `j` = yearly. Personal parameters (tax class, state, children, church, health surcharge) come from Settings → Bruno; the net/month and net/year headline animate as you type. Not tax advice.",
    arguments: [{ name: "€", required: true, description: "Gross amount; optional suffix `m` (monthly) or `j` (yearly).", default: undefined }],
    flags: [],
    examples: [
      { input: "bruno 4500", result: "Net pay for €4500 gross with your Settings defaults." },
