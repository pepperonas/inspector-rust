...
### Screen recording (`screen_record.rs`, v0.81.0)

Triggered by **`Ctrl+Shift+R`** (literal Control on every OS, registered in `hotkey.rs` alongside the screenshot/OCR shortcuts — dispatched to the **main thread** via `run_on_main_thread` because it builds a window). Records a screen **region** to an **MP4 (H.264)** with the **same workflow on macOS and Windows 11**. Flow: hotkey → `commands::screen_record_open_overlay` builds a fullscreen transparent `record-overlay` window (`RECORD_OVERLAY_LABEL`) covering the primary monitor → `RecordOverlay.tsx` runs three phases (**select** marquee-drag → **configure** System/Mic audio toggles + Record → **countdown** 3-2-1, then goes fully transparent for ~150 ms so the first frames are clean) → `start_screen_record(region, audio)` spawns ffmpeg, closes the overlay, and opens the `record-stop` bar (`RECORD_STOP_LABEL`, `RecordStopBar.tsx` — pulsing red dot + elapsed timer + Stop). **Stop** → `stop_screen_record` writes `q` to ffmpeg's stdin (graceful finalise, else kill after 5 s), reveals the MP4 in Finder/Explorer (`reveal_in_file_manager`), emits `"recording-saved"`. Region is sent in **physical pixels** (the overlay converts its CSS marquee rect × `devicePixelRatio`; W/H floored to even for `yuv420p`).
...
- **macOS:** `avfoundation` — `resolve_args` runs `ffmpeg -f avfoundation -list_devices true -i ""`, the **pure** `parse_avf_devices`/`pick_*_index` parsers pick the screen-capture video index + the BlackHole-style system-loopback + the mic, region via `-vf crop=W:H:X:Y`. **Windows:** `gdigrab` (`-offset_x/-offset_y/-video_size`) + `dshow` audio (parsed by `parse_dshow_audio`/`pick_dshow_*`) — **runtime-unverified**. Two audio tracks are mixed with `amix=inputs=2`. Output: `libx264 -preset ultrafast -pix_fmt yuv420p -movflags +faststart` → `dirs::download_dir()/Recording-<ts>.mp4`.
- **State:** `RecordState { Mutex<Option<Active>> }` (managed Tauri state); `Active { child, out }`. The pure arg-builders (`build_args_macos`/`build_args_windows`) + device parsers are unit-tested (8 tests, no ffmpeg needed). **Caveats:** ffmpeg must be installed; system audio needs a loopback device (macOS: BlackHole); macOS may prompt ffmpeg for its own Screen-Recording TCC the first time. IPC: `screen_record_open_overlay`, `cancel_record_overlay`, `start_screen_record`, `stop_screen_record`, `is_recording`. Frontend wrappers in `ipc.ts`; both windows routed in `main.tsx` and listed in each `capabilities/default.json`.
...
`meme [query]` browses a folder of GIFs/images and copies the selected one on Enter. `meme.rs::scan` **recursively** walks `meme.dir` (setting; default `/Users/martin/My Drive/media/memes`) collecting image files (gif/png/jpg/jpeg/webp/bmp/apng), each a `MemeEntry { name (stem), category (parent folder), path }`, sorted by category+name. `meme` is a **whole-list override** (like `kill`): `App.tsx` `isMemeMode` fetches the library once (`list_memes`), `lib/meme.ts::matchMemes` fuzzy-ranks it against the arg (name match outranks category; exact > prefix > infix > 3+-char subsequence), and the rows replace `combined`. The selected meme **previews animated** in `PreviewPanel` via `convertFileSrc(path)` in an `<img>` (the asset-protocol scope in each `tauri.conf.json` must include the meme dir — `$HOME/My Drive/media/memes/**`). Enter → `copy_meme(path)` → `meme::copy_to_clipboard`, which on **macOS writes an `NSURL` file-URL to `NSPasteboard`** (raw `objc2` msg_send) so pasting into a chat app / Finder keeps the **animation** (not a flattened still); other platforms fall back to `set_image` of the decoded bytes. IPC: `list_memes`, `copy_meme`.
